<template>
  <main class="container container--full">
    <div class="row">
      <div class="col col-lg-5 col-md-5 d-none d-md-block layout__left-bg minvh-100" />

      <div class="col col-lg-7 col-md-7 cols-12 minvh-100 d-flex flex-column justify-content-between">
        <div class="d-flex justify-content-end pt-1">
          <a
            href="https://www.botcoin.mt/register/"
            target="_blank"
            v-waves
            class="btn btn-primary-outline btn-small mr-1">
            <i class="fas fa-user-plus mr-50"></i>
            <span>Register</span>
          </a>
          <router-link
            :to="{ name: 'SignIn'}"
            tag="a"
            v-waves
            class="btn btn-primary btn-small">
            <i class="fas fa-key mr-50" />
            <span>Sign In</span>
          </router-link>
        </div>

        <div class="row">
          <div class="col col-lg-6 offset-lg-3 col-md-8 offset-md-2 col-sm-6 offset-sm-3 col-xs-10 offset-xs-1">

            <form
              @submit="changePassword($event)"
              novalidate="true"
              class="d-flex flex-column pb-1">

              <div class="d-flex justify-content-center align-items-center mb-3">
                <h3 class="m-0 text-uppercase">
                  Set New Password
                </h3>
              </div>

              <div class="input">
                <label
                  for="password"
                  class="d-flex align-items-center input__label">
                  <span>New Password</span>
                </label>
                <input
                  id="password"
                  type="password"
                  class="input__field input__field--medium"
                  :class="isInvalid('password') ? 'input__field--invalid' : null"
                  placeholder="············"
                  autocomplete="off"
                  v-model="userData.password"
                  @input="clearValidation('password')" />
                <label
                  for="password"
                  v-for="(error, index) in validations.password"
                  :key="`password-error-${index}`"
                  v-show="!error.valid"
                  class="input__error">
                  <i class="fas fa-exclamation-triangle mr-50" />
                  <span>{{ error.message }}</span>
                </label>
              </div>

              <div class="input">
                <label
                  for="password_re"
                  class="d-flex align-items-center input__label">
                  <span>Repeat New Password</span>
                </label>
                <input
                  id="password_re"
                  type="password"
                  class="input__field input__field--medium"
                  :class="isInvalid('password') ? 'input__field--invalid' : null"
                  placeholder="············"
                  autocomplete="off"
                  onpaste="return false"
                  v-model="userData.password_re"
                  @input="clearValidation('password_re')" />
                <label
                  for="password_re"
                  v-for="(error, index) in validations.password_re"
                  :key="`password_re-error-${index}`"
                  v-show="!error.valid"
                  class="input__error">
                  <i class="fas fa-exclamation-triangle mr-50" />
                  <span>{{ error.message }}</span>
                </label>
              </div>

              <button
                type="submit"
                v-waves
                class="btn btn-medium btn-primary btn-block">
                Submit
              </button>
            </form>

          </div>
        </div>

        <div /> <!-- ! spacer for flex-column -->
      </div>
    </div>

    <app-preloader :show="passwordChangeResponse" />
  </main>
</template>

<script>
import { eventBus } from '../../main'

export default {
  name: 'PasswordChange',
  data() {
    return {
      userData: {
        email: '',
        code: '',
        password: '',
        password_re: '',
      },
      validations: {
        password: {
          required: {
            valid: true,
            message: 'Password field is required.',
          },
        },
        password_re: {
          required: {
            valid: true,
            message: 'Please type your password again',
          },
        },
      },
    }
  },
  computed: {
    passwordChangeResponse() {
      return this.$store.state.auth.responses.passwordChange
    },

    routerData() {
      return this.$store.state.auth.routerData
    },
  },
  created() {
    this.completePassChangeForm()
  },
  mounted() {
    eventBus.$on('showToast/PasswordChange', data => { this.showToast(data) })
  },
  beforeDestroy() {
    eventBus.$off('showToast/PasswordChange')
  },
  methods: {
    completePassChangeForm() {
      if (Object.keys(this.routerData).length > 0) {
        this.userData.email = this.routerData.email
        this.userData.code = this.routerData.code
      }
    },

    validateForm(event) {
      event.preventDefault()

      const self = this
      let results = []

      return new Promise(resolve => {
        function testPassword() {
          const password = self.userData.password
          const validations = self.validations.password

          // Required
          if (password.length === 0) {
            validations.required.valid = false
          } else {
            validations.required.valid = true
          }
        }

        function testPasswordRe() {
          /* eslint-disable camelcase */
          const password_re = self.userData.password_re
          const validations = self.validations.password_re

          // Required
          if (password_re.length === 0) {
            validations.required.valid = false
          } else {
            validations.required.valid = true
          }
        }

        function checkResults() {
          results = []

          Object.keys(self.validations).forEach(type => {
            const target = self.validations[type]

            Object.keys(target).forEach(param => {
              results.push(target[param].valid)
            })
          })
        }

        testPassword()
        testPasswordRe()
        checkResults()

        results.some(el => !el) ? resolve(false) : resolve(true)
      })
    },

    clearValidation(type) {
      const target = this.validations[type]

      Object.keys(target).forEach(key => {
        target[key].valid = true
      })
    },

    isInvalid(type) {
      const target = this.validations[type]
      const errors = []

      Object.keys(target).forEach(key => {
        errors.push(target[key].valid)
      })

      return errors.some(el => !el)
    },

    changePassword(event) {
      this.validateForm(event).then(result => {
        if (result) {
          // Back-end requires exactly this format of data 🤷‍♂️
          let formData = new FormData()

          formData.append('json', 1)
          formData.append('login[email]', this.userData.email)
          formData.append('code', this.userData.code)
          formData.append('password', this.userData.password)
          formData.append('password_re', this.userData.password_re)

          this.$store.dispatch('auth/changePassword', formData)
        }
      })
    },

    showToast(data) {
      this.$toast(data.message, {
        type: data.type
      })
    }
  }
}
</script>
