<template>
  <main class="container container--full">
          ...
          <div class="col col-lg-6 offset-lg-3 col-md-8 offset-md-2 col-sm-6 offset-sm-3 col-xs-10 offset-xs-1">
            ...
            <form
              ...
              <div class="mb-1 text-center">
                ...
                <p>Please enter your number from Authenticator.</p>
              </div>
              ...
              <div class="input">
                ...
                <input
                  id="code"
                  type="text"
                  class="input__field input__field--medium text-center"
                  maxlength="6"
    ...
    <app-preloader :show="verifyCodeResponse" />
  </main>
</template>
...
export default {
  ...
  computed: {
    verifyCodeResponse() {
      return this.$store.state.auth.responses.verifyCode
    },
  },
  mounted() {
    eventBus.$on('showToast/OtpVerification', data => { this.showToast(data) })
  },
  beforeDestroy() {
    eventBus.$off('showToast/OtpVerification')
  },
  methods: {
    validateForm(event) {
    ...
    verifyCode(event) {
      this.validateForm(event).then(result => {
        if (result) {
          ...
          formData.append('form[mobile_token]', this.userData.code)

          this.$store.dispatch('auth/verifyCode', formData)
        }
      })
    },
