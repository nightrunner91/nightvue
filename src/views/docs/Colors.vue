<template>
  ...
  <section>
    <Heading
      ...
      :title="'Цвета'" />
    <p>
      Работа с цветами в NightVue будет привычна пользователю Booststrap. В общем и целом системы похожи, но есть небольшие различия, которые опытный разработчик наверняка определит самостоятельно.
    </p>
    ...
    <Heading
      ...
      :title="'Карты цветов'" />
    <p>
      Карта <code>$colors</code>, расположенная в файле <code>_varaibles.scss</code> насчитывает 12 стандартных цветов. Беря их за основу, можно настроить цвета темы, о которых пойдёт речь далее. Разумеется, вы можете изменять, удалять или добавлять новые цвета в эту карту.
    </p>
    ...
    <p>
      Также вам доступен ахроматический ряд из 12 цветов, где крайними выступают белый и чёрный цвета, а в промежутке между ними — оттенки серого, различающихся по яркости. Длину этого ряда можно изменять, редактируя переменную <code>$shadesNum: 10</code> в файле <code>_varaibles.scss</code>. Миксин автоматически сгенерирует заданное количество промежуточных цветов в карте <code>$grays</code>.
    </p>
    <ul class="d-flex list-unstyled colors my-2">
    ...
    <p class="mb-2">
      К каждому из цветов в библиотеке применяются модификаторы, изменяющие их яркость. Благодаря этому палитра цветов заметно расширяется. Это достигается с помощью Sass миксинов, чтобы осветлять или затемнять цвета с помощью встроенной функции Sass <code>mix()</code>.
    </p>
    <ul
      v-for="color in colors"
      :key="`variants-${color}`"
    ...
    <Heading
      :tag="'h3'"
      :id="'color-tags'"
      :title="'Зарезервированные классы цветов в HTML'" />
    <p>
      Практика показывает, что наиболее часто возникает необходимость изменить цвет блока, цвет текста и фон заливки SVG-элемента. Для этих целей в NightVue зарезервирован ряд классов <code>bg-${color}</code>, <code>clr-${color}</code> и <code>fill-${color}</code> для придания нужного цвета фону, тексту и заливке SVG-элемента соответственно. В переменную <code>${color}</code> можно подставлять любой цвет из вышеназванных карт. При необходимости вы можете указать модификатор <code>-light-${level}</code> или <code>-dark-${level}</code> для уточнения уровня цвета. Вот как это выглядит:
    </p>
    ...
    <p>
      Мы настоятельно рекомендуем использовать этот способ как можно чаще, т.к. это ощутимо сокращает количество скомпилированного CSS кода. Предпочительней использовать по максимуму классы с уже описанными свойствами, чем генерировать новые.
    </p>
    ...
    <Heading
      :tag="'h3'"
      :id="'color-map-get'"
      :title="'Доступ к картам цветов через модуль map-get()'" />
    <p>
