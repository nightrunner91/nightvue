<template>
  ...
  <section>
    ...
    <h2>
      Использование карты цветов
    </h2>
    <p>
      Наконец, вы можете воспользоваться встроенным Sass модулем <code>map.get()</code> для прямого доступа к картам <code>$colors</code>, <code>$theme-colors</code> и <code>$levels</code> в файле <code>_varaibles.scss</code>. Или если по каким-либо причинам первые два метода вам не подходят. Мы, однако, рекомендуем использовать их хотя бы по той причине, что миксины NightVue как раз и делают то, что вы намереваетесь делать самостоятельно каждый раз при работе с цветами. Подробнее о работе с картами Sass можно ознакомиться в <a rel="nofollow" target="_blank" href="https://sass-lang.com/documentation/values/maps">официальной документации</a>.
    </p>
    <p>
