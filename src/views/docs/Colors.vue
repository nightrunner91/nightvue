<template>
  ...
  'purple':    #682cab,
    ...
    <ol class="pl-2">
      <li>
        Для использования цветов в HTML-документе используйте классы вида <code>bg-red</code>, <code>clr-red</code> или <code>fill-red</code> для придания нужного цвета фону, тексту или SVG элементу соответственно. Примеры: <pre class="language-html"><code>
          <div class="bg-red">ololo</div>
          </code></pre>
      </li>
    </ol>
    ...
    <p>Пользователю доступен ахроматический ряд, длину которого можно регулировать. По умолчанию настроен ряд из 16 таких цветов, различающихся по яркости.</p>
    <ul class="d-flex list-unstyled colors my-3">
    ...
    <p class="mb-3 test">
      К каждому из цветов в библиотеке (за исключением чёрного и белого) применяется ряд модификаторов, изменяющих его яркость. Для этого используются миксины и встроенный модули SASS <code>mix()</code>. Подробнее о работе с цветами в SASS можно ознакомиться в <a rel="nofollow" target="_blank" href="https://sass-lang.com/documentation/modules/color">документации</a>.
    </p>
    <ul
      v-for="(color, value) in colors"
